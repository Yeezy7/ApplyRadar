...
export default function ResumePage() {
  ...
  const loadResumes = useCallback(async () => {
    ...
    try {
      ...
      showError(`加载失败: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSetDefault = async (id: string) => {
    try {
      ...
      showError(`操作失败: ${e instanceof Error ? e.message : String(e)}`);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pb-8 pt-4">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">简历管理</h1>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2 text-xs text-gray-400">
                    {resume.education && resume.education.length > 0 && (
                      <span>
                        {resume.education.length} 段教育经历
                      </span>
                    )}
                    {resume.work_experience &&
                      resume.work_experience.length > 0 && (
                        <span>
                          {resume.work_experience.length} 段工作经历
                        </span>
                      )}
                ...
                <div className="flex items-center gap-1 ml-4">
                  ...
                  <button
                    ...
                    className="p-2 text-gray-400 hover:text-stone-700 hover:bg-stone-100 rounded-lg transition"
                    title="编辑"
                  >
...
function ResumeForm({ resume, onSaved, onCancel, showError, showSuccess }: ResumeFormProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!resumeId) {
      ...
      setSaving(true);
      try {
        const created = await createResume(form);
        resumeId = created.id;
        setEditingResumeLocal(created);
      } catch (err) {
        showError(`创建失败: ${err instanceof Error ? err.message : String(err)}`);
        setSaving(false);
        return;
      }
      setSaving(false);
  ...
  const setEditingResumeLocal = (r: Resume) => {
    // Update local form state without prop drilling
    setForm(r);
  };

  const handleParse = async () => {
    if (!resume?.id && !form.id) {
      showError("请先保存简历");
      return;
    }
    ...
    setParsing(true);
    try {
      const updated = await parseResumePdf(id);
      setForm(updated);
      showSuccess("简历已解析");
    } catch (e) {
      showError(`解析失败: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
      setParsing(false);
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between bg-white rounded-2xl border border-gray-100 p-4">
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 px-4 py-2 text-sm text-stone-700 bg-stone-50 hover:bg-stone-100 rounded-2xl cursor-pointer transition-colors">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={onCancel}
            className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-2xl transition-colors"
          >
            取消
          </button>
          <button
      ...
      >
        ...
        {(!form.education || form.education.length === 0) && (
          <p className="text-sm text-gray-400 text-center py-4">暂无教育经历</p>
        )}
      </FormSection>
      ...
      >
        ...
        {(!form.work_experience || form.work_experience.length === 0) && (
          <p className="text-sm text-gray-400 text-center py-4">暂无工作经历</p>
        )}
      </FormSection>
      ...
      >
        ...
        {(!form.projects || form.projects.length === 0) && (
          <p className="text-sm text-gray-400 text-center py-4">暂无项目经历</p>
        )}
      </FormSection>
      ...
      >
        ...
        {(!form.certifications || form.certifications.length === 0) && (
          <p className="text-sm text-gray-400 text-center py-4">暂无证书</p>
        )}
      </FormSection>
      ...
      >
        <textarea
          ...
          rows={5}
          className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-stone-500/20 focus:border-stone-400 transition-all resize-none"
      ...
      <div className="flex justify-end gap-3 pb-4">
        <button
          onClick={onCancel}
          className="px-5 py-2.5 text-sm text-gray-600 hover:bg-gray-100 rounded-2xl transition-colors"
        >
          取消
        </button>
        <button
...
function EducationItem({
  ...
  return (
    <div className="p-4 bg-gray-50 rounded-xl space-y-3 relative">
      <button
        onClick={onRemove}
        className="absolute top-2 right-2 p-1 text-gray-400 hover:text-red-500 rounded transition"
      >
    ...
    <div className="p-4 bg-gray-50 rounded-xl space-y-3 relative">
      ...
      <div>
        ...
        <textarea
          ...
          rows={3}
          className="w-full px-3.5 py-2.5 bg-white border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-stone-500/20 focus:border-stone-400 transition-all resize-none"
        />
      </div>
    </div>
  );
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          {data.tech_stack?.map((t, i) => (
            <span key={i} className="inline-flex items-center gap-1 px-2 py-0.5 bg-white border border-gray-200 text-gray-600 text-xs rounded-full">
              {t}
              <button
                onClick={() => {
        ...
        <SkillInput
          onAdd={(skill) =>
            onChange({ ...data, tech_stack: [...(data.tech_stack || []), skill] })
          }
...
function CertItem({
  ...
  return (
    <div className="p-4 bg-gray-50 rounded-xl grid grid-cols-3 gap-3 relative">
      <button
        onClick={onRemove}
        className="absolute top-2 right-2 p-1 text-gray-400 hover:text-red-500 rounded transition"
      >
