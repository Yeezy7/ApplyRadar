...
import { pushLogService, confirmDelete } from "../services";
import type { PushLog } from "../services/pushLogService";
...
export default function PushPage() {
  ...
  const loadLogs = useCallback(async () => {
    setLoading(true);
    try {
      const data = await pushLogService.listPushLogs(500);
      setLogs(data);
    } catch (e) {
  ...
  const handleClear = async () => {
    const ok = await confirmDelete("确定要清空所有推送记录吗？");
    if (!ok) return;
    try {
      await pushLogService.clearPushLogs();
      setLogs([]);
    } catch (e) {
  ...
  const filtered = useMemo(() => {
    ...
    return logs.filter((r) => r.pushType === typeFilter);
  }, [logs, typeFilter]);

  const typeCounts = useMemo(() => {
    const counts: Record<string, number> = {};
    for (const r of logs) {
      counts[r.pushType] = (counts[r.pushType] || 0) + 1;
    }
    return counts;
  }, [logs]);
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="divide-y divide-gray-50">
            {filtered.map((log) => {
              const cfg = getConfig(log.pushType);
              const Icon = cfg.icon;
              const isExpanded = expandedId === log.id;
              const hasExpandableContent = (log.detail && log.detail.length > 50) || log.errorMessage;
              return (
                <div key={log.id}>
                  <div
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isExpanded && log.errorMessage && (
                        <p className="text-xs text-red-500 mt-1.5 p-2 bg-red-50 rounded-lg break-all">
                          {log.errorMessage}
                        </p>
                      )}
                    </div>
